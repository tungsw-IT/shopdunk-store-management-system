...
$err = '';
$selected_brand = $_POST['company_name'] ?? 'Apple';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($ram < 0 || $rom < 0 || $display_size < 0 || $battery < 0 || $stock_quantity < 0 || $price < 0 || $old_price < 0) {
        ...
        $checkStmt = $conn->prepare("SELECT imei_number FROM mobile WHERE imei_number = ? LIMIT 1");
        $checkStmt->bind_param('s', $imei_number);
        $checkStmt->execute();
        $exists = $checkStmt->get_result()->fetch_assoc();
        $checkStmt->close();

        if ($exists) {
            $err = 'IMEI đã tồn tại. Vui lòng nhập mã IMEI khác.';
        } else {
            $stmt = $conn->prepare("INSERT INTO mobile
                (company_name, company_series, model_no, imei_number, `ram(GB)`, `rom(GB)`, `display_size(inchi)`, display_quality, processor, `battery_capacity(mah)`, color, stock_quantity, price, old_price)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param(
                'ssssiidssisidd',
                $company_name,
            ...
            $imagePaths = save_uploaded_product_images($_FILES['product_images'] ?? [], $imei_number);
            insert_product_images_mysqli($conn, $imei_number, $imagePaths);
...
<head>
  ...
  <script>
    ...
    window.addEventListener('DOMContentLoaded', function() {
      ...
      fillSeries(brandSelect.value, <?= json_encode($_POST['company_series'] ?? '', JSON_UNESCAPED_UNICODE) ?>);
  ...
  <div class="form-container">
    ...
    <form method="post" enctype="multipart/form-data" autocomplete="off">
      ...
      <div class="form-group">
        ...
        <input id="model_no" name="model_no" value="<?= htmlspecialchars($_POST['model_no'] ?? '') ?>" required>
      </div>
      <div class="form-group">
        <label for="imei_number">IMEI Number:</label>
        <input id="imei_number" name="imei_number" value="<?= htmlspecialchars($_POST['imei_number'] ?? '') ?>" required>
      </div>
      <div class="form-group">
        ...
        <select id="ram" name="ram" required>
          ...
          <?php foreach ($ram_options as $ram_option): ?>
            <option value="<?= $ram_option ?>" <?= (string)($ram_option) === (string)($_POST['ram'] ?? '') ? 'selected' : '' ?>><?= $ram_option ?>GB</option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="form-group">
        ...
        <select id="rom" name="rom" required>
          ...
          <?php foreach ($rom_options as $rom_value => $rom_label): ?>
            <option value="<?= $rom_value ?>" <?= (string)($rom_value) === (string)($_POST['rom'] ?? '') ? 'selected' : '' ?>><?= htmlspecialchars($rom_label) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="form-group">
        <label for="display_size">Kích thước màn (inchi):</label>
        <input id="display_size" name="display_size" type="number" step="0.01" min="0" value="<?= htmlspecialchars($_POST['display_size'] ?? '') ?>" required>
      </div>
      <div class="form-group">
        <label for="display_quality">Chất lượng màn hình:</label>
        <input id="display_quality" name="display_quality" value="<?= htmlspecialchars($_POST['display_quality'] ?? '') ?>" required>
      </div>
      <div class="form-group">
        <label for="processor">Vi xử lý:</label>
        <input id="processor" name="processor" value="<?= htmlspecialchars($_POST['processor'] ?? '') ?>" required>
      </div>
      <div class="form-group">
        <label for="battery">Pin (mAh):</label>
        <input id="battery" name="battery" type="number" min="0" value="<?= htmlspecialchars($_POST['battery'] ?? '') ?>" required>
      </div>
      <div class="form-group">
        <label for="color">Màu sắc:</label>
        <input id="color" name="color" value="<?= htmlspecialchars($_POST['color'] ?? '') ?>" required>
      </div>
      <div class="form-group">
        <label for="stock_quantity">Số lượng:</label>
        <input id="stock_quantity" name="stock_quantity" type="number" min="0" value="<?= htmlspecialchars($_POST['stock_quantity'] ?? '0') ?>" required>
      </div>
      <div class="form-group">
        <label for="price">Giá:</label>
        <input id="price" name="price" type="number" step="10000" min="0" value="<?= htmlspecialchars($_POST['price'] ?? '') ?>" required>
      </div>
      <div class="form-group">
        <label for="old_price">Giá cũ (nếu có):</label>
        <input id="old_price" name="old_price" type="number" step="10000" min="0" value="<?= htmlspecialchars($_POST['old_price'] ?? '') ?>">
      </div>
