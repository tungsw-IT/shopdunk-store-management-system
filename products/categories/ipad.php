...
function getIpadProductImage($product) {
    $folder = '../../images/';
...
$stmt = $pdo->prepare("SELECT * FROM mobile WHERE company_name = 'Apple' AND company_series LIKE ? ORDER BY price ASC");
$stmt->execute(['%iPad%']);
...
<div class="page-shell">
  <section class="slider-shell">
    <div class="slider" id="pageSlider">
      ...
      <div class="track" id="pageTrack">
        <?php foreach ($slides as $index => $slide): ?>
          <div class="slide"><img src="<?= htmlspecialchars($slide) ?>" alt="iPad banner <?= $index + 1 ?>"></div>
        <?php endforeach; ?>
      </div>
    ...
    <div class="dots">
      <?php foreach ($slides as $index => $slide): ?>
        <button class="dot<?= $index === 0 ? ' active' : '' ?>" type="button" data-slide="<?= $index ?>"></button>
      <?php endforeach; ?>
    </div>
  </section>

  <nav class="filter-bar"><a href="ipad.php" class="tab active">Tất cả</a></nav>
    ...
    <?php foreach ($displayedProducts as $product): $slug = urlencode($product['model_no']); $config = $product['ram(GB)'] . 'GB + ' . $product['rom(GB)'] . 'GB'; $isWishlisted = in_array($product['imei_number'], $userWishlist); $heartClass = $isWishlisted ? 'fa-solid fa-heart' : 'fa-regular fa-heart'; $heartColor = $isWishlisted ? '#e53935' : '#ff4444'; ?>
      <div class="product-card hover-hand" style="position:relative;">
        ...
        <a href="../detail/product-detail.php?model_no=<?= $slug ?>" class="product-card-link">
          <img src="<?= htmlspecialchars(getIpadProductImage($product)) ?>" alt="<?= htmlspecialchars($product['company_series'] . ' ' . $product['model_no']) ?>">
